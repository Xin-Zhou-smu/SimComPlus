...
from utils import mini_batches_test, mini_batches_train_handfeatures, mini_batches_test_handfeatures
...
def evaluation_model(data, params):
    ...
    pad_msg, pad_code, labels, dict_msg, dict_code, data_hand_cat, data_hand_num = data


    # batches = mini_batches_test(X_msg=pad_msg, X_code=pad_code, Y=labels)
    batches = mini_batches_test_handfeatures(X_msg=pad_msg, X_code=pad_code, X_cat_feat=data_hand_cat, X_num_feat=data_hand_num, Y=labels)
    ...
    df.to_csv(params.save_dir+'/test_com_score.csv', index=False, sep=',')
